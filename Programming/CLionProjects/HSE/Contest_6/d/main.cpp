...
template <typename T>
MathVector <T> operator + (const MathVector <T>& first, const MathVector <T>& second) {
    MathVector <T> ans(first.Dimension());
...
template <typename T>
MathVector <T>& operator += (MathVector <T>& first, const MathVector <T>& second) {
    for (size_t i = 0; i < first.Dimension(); i++) {
